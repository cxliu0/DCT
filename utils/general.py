...
def compute_loss_dual_pred(p, p_ori, targets, model):  # predictions, targets, model
    device = targets.device
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    lbox_ori, lobj_ori = torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors = build_targets(p_ori, targets, model)  # targets
    h = model.hyp  # hyperparameters
    ...
    balance = [4.0, 1.0, 0.4] if np == 3 else [4.0, 1.0, 0.4, 0.1]  # P3-5 or P3-6
    balance = [4.0, 1.0, 0.5, 0.4, 0.1] if np == 5 else balance
    for i, (pi, pi_ori) in enumerate(zip(p, p_ori)):  # layer index, layer predictions
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
        tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
        tobj_ori = torch.zeros_like(pi_ori[..., 0], device=device)  # target obj
        ...
        if n:
            ...
            # *** ori pred loss
            ps_ori = pi_ori[b, a, gj, gi]  # prediction subset corresponding to targets

            # Regression
            pxy_ori = ps_ori[:, :2].sigmoid() * 2. - 0.5
            pwh_ori = (ps_ori[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
            pbox_ori = torch.cat((pxy_ori, pwh_ori), 1).to(device)  # predicted box
            giou_ori = bbox_iou(pbox_ori.T, tbox[i], x1y1x2y2=False, CIoU=True)  # giou(prediction, target)
            lbox_ori += (1.0 - giou_ori).mean()  # giou loss

            # Objectness
            tobj_ori[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou_ori.detach().clamp(0).type(tobj.dtype)  # giou ratio

        lobj += BCEobj(pi[..., 4], tobj) * balance[i]  # obj loss
        lobj_ori += BCEobj(pi_ori[..., 4], tobj_ori) * balance[i]  # obj loss
    ...
    lbox_ori *= h['giou'] * s
    lobj_ori *= h['obj'] * s * (1.4 if np >= 4 else 1.)
    bs = tobj_ori.shape[0]  # batch size
    loss_ori = lbox_ori + lobj_ori
    return loss * bs, loss_ori * bs, torch.cat((lbox, lobj, lcls, loss)).detach(), torch.cat((lbox_ori, lobj_ori, lcls, loss_ori)).detach()
...
def non_max_suppression1(prediction, conf_thres=0.4, iou_thres=0.5, merge=False, classes=None, agnostic=False):
    """Performs Non-Maximum Suppression (NMS) on inference results
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 5:]*x[:, 4:5] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = (x[:, 5:]*x[:, 4:5]).max(1, keepdim=True)
            x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]
...
def fitness(x):
    ...
    w = [0.0, 0.0, 0.1, 0.4, 0.5]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95, ADA]
    return (x[:, :5] * w).sum(1)
